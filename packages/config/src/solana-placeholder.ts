 ...
 *
 * Shared across all WaffleFinance services (coordinator, relayer, resolver)
 * so placeholder detection is consistent everywhere.
 *
 * When the program is a placeholder, Solana settlement flows must be
 * disabled and operators should see an explicit warning.
 */
...
/**
 * Checks the Solana program ID, logs an appropriate warning or info
 * message, and returns the config status.  Call this at service startup
 * so operators see an explicit warning when Solana is not yet configured.
 *
 * Uses `console.warn` / `console.log` rather than a structured logger
 * so it can be called before the logger is initialised.
 */
export function logSolanaStatus(programId: string | undefined): SolanaConfigStatus {
  ...
  if (status === "placeholder") {
    ...
    const preview =
      programId!.length > 16 ? programId!.substring(0, 16) + "…" : programId!;
